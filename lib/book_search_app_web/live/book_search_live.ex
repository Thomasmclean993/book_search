defmodule BookSearchAppWeb.BookSearchLive do
  ...
  def handle_event("search", %{"query" => query}, socket) do
    ...
    {:noreply,
     assign(socket, loading: true, error: nil, query: query, books: [], selected_book: nil)}
  end

  def handle_event("select_book", %{"key" => book_key}, socket) do
    book = Enum.find(socket.assigns.books, fn book -> book[:key] == book_key end)

    recommendations =
      if book do
        {:ok, recs} = TasteDive.get_recommendations(book[:title])
        recs
      else
        []
      end

    {:noreply, assign(socket, selected_book: book, recommendations: recommendations)}
  end

  def handle_info({:search_books, query}, socket) do
    case OpenLibrary.fetch_books(query) do
      {:ok, books} ->
